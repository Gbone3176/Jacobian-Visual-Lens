...
Then run `run-single` with explicit paths. Keep `--allow-model-run` absent for no-model static fixture checks and present only when model/GPU execution is authorized.
